...
def remove_silence(df):
    ...
    for index in tqdm(df.index):
        ...
        audio = AudioSegment.from_wav(f"{AUDIO_PATH}/{id_i}")
...
def audio_augmentation(df):
    ...
    for index in tqdm(df.index):
        row = df.loc[index]
        file_path = f'{ROOT_DIR}/data/processed/Audio/Clean/{row.id}'
        ...
        if id_hi not in aug_files:
            ...
            for wav_i in wav_list:
                ...
                sf.write(f'{AUGMENTED_PATH}/{id_i}', wav_i[1], sample_rate)
        else:
            for id_i in [id_wn, id_dp, id_hi]:
                row['id'] = id_i
                row['audio_type'] = id_i[5:7]
                df_aug = pd.concat([df_aug, pd.DataFrame(row).T])
