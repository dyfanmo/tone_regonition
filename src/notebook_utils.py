...
from .utils import text_to_tone, get_melspectrogram_db, specgram_to_image, get_audio_path, change_pitch, \
    convert_index_to_strings, FIGURE_PATH, AUDIO_PATH, DATA_PATH, CLEAN_PATH
...
def display_tone_dist(chinese_words, df, file_path='tone_distribution.png'):
    ...
    chinese_df['tones'] = chinese_df['word'].apply(lambda word: text_to_tone(word))
    tone_count0 = chinese_df['tones'].value_counts()
    tone_per0 = tone_count0 / tone_count0.sum()
    tone_count1 = df['tones'].value_counts()
    tone_per1 = tone_count1 / tone_count1.sum()

    fig, axs = plt.subplots(1, 2, figsize=(17, 8), sharex=True)
    axs[0].set_ylabel('Tones')
    fig.text(0.5, 0.04, 'Per %', ha='center')

    for i, data in enumerate([tone_per0, tone_per1]):
        str_index = convert_index_to_strings(data.index)
        axs[i].barh(str_index, data)
        t = axs[i].set_yticks(str_index)
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            for i in range(n_cols * n_rows):
                audio = df.sample(1)
                file_path = AUDIO_PATH + audio.id.iloc[0]
                wav, rate = librosa.load(file_path, sr=None)
                index = n_cols * row + col
                plt.subplot(n_rows, n_cols, index + 1)
            librosa.display.waveplot(wav, rate)  # plot wave
            plt.title(f'Tone: {audio.tones.iloc[0]}')
            plt.tight_layout()
    plt.savefig(FIGURE_PATH + '/wave_plots.png')
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            for i in range(n_cols * n_rows):
                audio = df.sample(1)
                file_path = AUDIO_PATH + audio.id.iloc[0]
                specgram = get_melspectrogram_db(file_path, duration=3)
                img = specgram_to_image(specgram)
                index = n_cols * row + col
                plt.subplot(n_rows, n_cols, index + 1)
            librosa.display.specshow(img, cmap='viridis')
            plt.title(f'Tone:{audio.tones.iloc[0]}')
    plt.savefig(FIGURE_PATH + 'mel_specgram.png')
...
def play_audio_quality(df):
    ...
    for ser_i in [high_quality, low_quality]:
        ...
        print(f'Tone: {ser_i.tones}\tSound Quality: {ser_i.sound_quality}\tAudio Type: {ser_i.audio_type}')
...
def play_audio_length(df):
    ...
    print(f'Word: {df.word}, Tone: {df.tones}')
    for file_path in [CLEAN_PATH, AUDIO_PATH]:
...
def play_anomalies(df, num=1):
    ...
    for i in range(num):
        ...
        for ser_i in [inlier, outlier]:
            file_path = get_audio_path(ser_i)
            print(f'Tone: {ser_i.tones}\tAnomaly: {ser_i.anomaly}\tAudio Type: {ser_i.audio_type}')
            display(ipd.Audio(file_path))
...
def play_aug(df):
    ...
    row = df.sample(1)
    file_path = CLEAN_PATH + row.id.iloc[0]
    wav, rate = librosa.load(file_path)

    wn = np.random.randn(len(wav))
    wav_wn = wav + 0.005 * wn
    wav_dp = change_pitch(wav, rate, deep=True)
    wav_hi = change_pitch(wav, rate, deep=False)
...
def display_aug(df):
    ...
    for i, wav_i in enumerate(wav_all):
        ...
        axs[i].plot(wav_i[1])
    plt.savefig(FIGURE_PATH + 'wave_plots_augmented.png')
...
def display_pca_tones(df):
    ...
    fig = plt.figure(figsize=(13, 9))
    ax = fig.add_subplot(1, 1, 1, projection='3d')
    for data in [tone1, tone2, tone3, tone4, tone5]:
        ax.scatter(data.PC1, data.PC2, data.PC3, alpha=0.2)
    ax.legend([1, 2, 3, 4, 5])
    ax.set_title('Tones')
...
def play_long_audio(df):
    ...
    for i in range(2):
        ...
        display(ipd.Audio(CLEAN_PATH + sample.id.iloc[0]))
